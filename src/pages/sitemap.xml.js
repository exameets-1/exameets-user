...
//   await dbConnect();
//   const jobs = await Job.find().select('slug');
...
//         .map(([section, items]) => generateUrls(section, items))
//         .join('')}
//     </urlset>
//   `;
// }
...
//       <!-- Other sections -->
//       ${Object.entries(allData)
//         .filter(([section]) => section !== 'previousYears') // Exclude previousYears as we handle it separately
//         .map(([section, items]) => generateUrls(section, items))
//         .join('')}
//     </urlset>
//   `;
